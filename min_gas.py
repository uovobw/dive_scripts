...
def minimum_gas(depth, bottle_size, change=NX_50_DEPTH, sac=DEFAULT_BOTTOM_SAC, ascent_speed=DECO_ASCENT_SPEED):
    ...
    total_liters = c * a * t
    bars_for_bottle = liters_to_bars(total_liters, bottle_size)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 1:
        ...
        ascent_speed = int(input("Ascent speed from start depth to final depth (meters/minute):  "))
        sac = int(input("Single diver Surface Air Consumption (SAC in literls):  "))
    min_gas = minimum_gas(start_depth, bottle_size, change=end_depth, sac=sac, ascent_speed=ascent_speed)
    if min_gas < MIN_MIN_GAS:
        print("WARNING: minimum gas of {} is less than recommended {} bars. Rounding to {}".format(min_gas, MIN_MIN_GAS, MIN_MIN_GAS))
        min_gas = MIN_MIN_GAS
